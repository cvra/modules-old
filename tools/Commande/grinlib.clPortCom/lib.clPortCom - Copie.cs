...
namespace lib.PortCom
{
	...
	//	private const bool UseInterrup = true;
		...
		public void Connect(string NumPort, int Baudrate)
		{
			...
			try
			{
				if (TimerDataSend != null && TimerDataSend.IsAlive)
				{
					try
					{
						TimerDataSend.Abort();
					...
					{
						Console.WriteLine("***Connect ThreadAbortException " + ex.Message);
					}
...
//				ComSerial = new SerialPort(NumPort, Baudrate, Parity.None, 8, StopBits.One);
		...
		private void EventSendTimer() //(Object stateInfo)
		{
			try
			{
				while (ComSerial != null && ComSerial.IsOpen)
				{
					Thread.Sleep(30);
					...
					{
						Console.WriteLine("EventSendTimer Exception " + ex.Message);
						CmtError++;
