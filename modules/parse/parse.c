 ...
 */
static int8_t
match_inst(parse_inst_t *inst, const char * buf, uint8_t nb_match_token, 
	   void * result_buf)
{
	uint8_t token_num=0;
	...
	while (token_p && (!nb_match_token || i<nb_match_token)) {
		...
		token_num ++;
		token_p = (parse_token_hdr_t *)pgm_read_pgmptr(&inst->tokens[token_num]);
		if (token_p)
			memcpy(&token_hdr, token_p, sizeof(token_hdr));
	}
...
int8_t
parse(parse_ctx_t ctx[], const char * buf)
{
	uint8_t inst_num=0;
	parse_inst_t * inst;
	const char * curbuf;
