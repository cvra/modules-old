...
struct token_ops {
	...
	int8_t (*parse)(parse_token_hdr_t *, const char *, void *);
	/** return the num of possible choices for this token */
	int8_t (*complete_get_nb)(parse_token_hdr_t *);
	/** return the elt x for this token (token, idx, dstbuf, size) */
	int8_t (*complete_get_elt)(parse_token_hdr_t *, int8_t, char *, uint8_t);
	/** get help for this token (token, dstbuf, size) */
	int8_t (*get_help)(parse_token_hdr_t *, char *, uint8_t);
};	
 ...
 * 0 (PARSE_SUCCESS).
 */
int8_t parse(parse_ctx_t ctx[], const char * buf);
