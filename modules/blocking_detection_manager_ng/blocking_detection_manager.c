...
void bd_ng_init(struct blocking_detection * bd)
{
	uint8_t flags;
	IRQ_LOCK(flags);
...
/* thresholds */
void bd_ng_set_current_thresholds(struct blocking_detection * bd,
...
/* speed threshold */
void bd_ng_set_speed_threshold(struct blocking_detection * bd,
			    uint16_t speed)
{
	uint8_t flags;
	IRQ_LOCK(flags);
...
void bd_ng_reset(struct blocking_detection * bd)
{
	uint8_t flags;
	IRQ_LOCK(flags);
	bd->cpt = 0;
...
void bd_ng_manage_from_speed_cmd(struct blocking_detection * bd,
...
/** function to be called periodically */
void bd_ng_manage_from_pos_cmd(struct blocking_detection * bd,
...
/** function to be called periodically */
void bd_ng_manage_from_cs(struct blocking_detection * bd, struct cs *cs)
{
	bd_manage_from_pos_cmd(bd, cs_get_filtered_feedback(cs), cs_get_out(cs));
}

/** get value of blocking detection */
uint8_t bd_ng_get(struct blocking_detection * bd)
{
	uint8_t ret, flags;
	IRQ_LOCK(flags);
